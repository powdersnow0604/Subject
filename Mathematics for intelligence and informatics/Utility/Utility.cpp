...
DataModel StandardScaler::transform(DataModel& dm)
{
	std::vector<std::vector<double>> scaled_input(dm.input.size(), std::vector<double>(dm.input[0].size(),0));
	auto mean_dm = mean(dm);
	auto stdev_dm = stdev(dm, mean_dm);
	
	for (size_t i = 0; i < dm.input.size(); ++i) {
		for (size_t j = 0; j < dm.input[0].size(); ++j) {
			scaled_input[i][j] = (dm.input[i][j] - mean_dm[j]) / stdev_dm[j];
		}
	}

	return DataModel(scaled_input, dm.target);
}

std::vector<double> BasicAi::Utility::mean(const DataModel& dm)
{
	std::vector<double> sum( dm.input[0].size(), 0. );
...
{
	std::vector<double> sum(dm.input[0].size(), 0);
	std::vector<double> stdev(dm.input[0].size(), 0);
	...
	std::transform(sum.begin(), sum.end(), stdev.begin(), [&dm] (const double& n) {return std::sqrt( n / dm.input.size()); });
